...
def animate_func(num):
    ...
    ax[2].clear()
    ########
    plot_3D_rectangle(110, 50,ax[3],2.0,   elevation=0, resolution=10, color='silver', x_center = -5, y_center = 20,order = 0)
    ax[3].plot3D([hiplx[num,0],kneelx[num,0],anklx[num,0]],[hiplx[num,1]+10,kneelx[num,1]+10,anklx[num,1]+10], [hiplx[num,2],kneelx[num,2],anklx[num,2]], zorder = 3,color='#FA525B',antialiased=True, linewidth=20,fillstyle='full',solid_capstyle='round')
    ax[3].plot3D([hiplx[num,0],kneelx[num,0]],[hiplx[num,1]+10,kneelx[num,1]+10], [hiplx[num,2]-2,kneelx[num,2]], color='#FA525B',antialiased=True,zorder = 3,linewidth=18,fillstyle='full',solid_capstyle='round')
    plot_3D_cylinder(5.5,-40, ax[3], elevation=6,resolution=20 ,  color='#c3c3c3', x_center=17, y_center=-2, order = 0)
    ax[3].plot3D([hiprx[0,0],kneerx[num,0],ankrx[num,0]],[hiprx[num,1]-8,kneerx[num,1]-9,ankrx[num,1]-8], [hiprx[num,2]+1+1,kneerx[num,2]+1,ankrx[num,2]+1], color='#19D3C5',antialiased=True,zorder = 1, linewidth=20,fillstyle='full',solid_capstyle='round')
    ...
    ax[3].plot3D([hiprx[0,0]+3,kneerx[num,0]],[hiprx[num,1]-8+1,kneerx[num,1]-9+1], [hiprx[num,2]-1+1+1,kneerx[num,2]+1], color='#19D3C5',antialiased=True,zorder = 2, linewidth=18,fillstyle='full',solid_capstyle='round')
...
# Setting Axes Limits
    ...
    #trial = emg.iloc[selected_trial]
    ax[1].plot([0,0], [170,-1070],linestyle='dotted',  color='#fa525b', linewidth=1)
    for r in range(7):
        #print(trial[muscle[r]].values)
        #predicted_line = ax[1].plot(time[start:end], emg_trial[start:end,r]*0.5-r*2,  color="#dbdbdd", linewidth=1)
        true_line = ax[1].plot(time[start+shift:end+shift], 0.95*trial[start+shift:end+shift,r]-r*170,  color="#dbdbdd", linewidth=1)
        #predicted_line = ax[1].plot(time[start+shift:end+shift], pred_trial[start+shift:end+shift,r]*0.04-r*2,  '--' , color="#dbdbdd", alpha = 0.8 , linewidth=1.8)
...
#####
    ...
    for r in range(7):
        #predicted_line = ax[1].plot(time[start:end], emg_trial[start:end,r]*0.5-r*2,  color="#dbdbdd", linewidth=1)
        true_line = ax[2].plot(time[start+shift:end+shift],  0.95*trial[start+shift:end+shift,r+7]-r*170,  color="#dbdbdd", linewidth=1)
        #predicted_line = ax[2].plot(time[start+shift:end+shift], pred_trial[start+shift:end+shift,r]*0.04-r*2,  '--' , color="#dbdbdd", alpha = 0.8 , linewidth=1.8)
...
#with open('Leftlegang503.npy', 'rb') as f:
#    ang = np.load(f, allow_pickle=True)
#hipl, kneel, ankl = to_coo(np.rad2deg(ang[0])-90,0 *ang[0], np.rad2deg(ang[1]-ang[0]))\n",
shadow_leg = True
if shadow_leg :
    ...
    #hiprX = hiplx[0] + 0 *hiplx
    #kneerX = kneelx[0] + 0 *hiplx
    #ankrX =  anklx[0] + 0 *hiplx
#hipr = hipl[0] + 0 *hipl
...
time = np.linspace(-1000*shift/stimfs, duration-1+1000*shift/stimfs,numDataPoints+window_width)
...
z_ = np.array([[0.9*hiplx[0,2]-3, 0.9*hiplx[0,2]-3, 0.9*hiplx[0,2]-3,0.9*hiplx[0,2]-3],[0.9*hiplx[0,2]-3, 1.1*hiplx[0,2]+4, 1.1*hiplx[0,2]+4,0.9*hiplx[0,2]-3], [0.9*hiplx[0,2]-3, 1.1*hiplx[0,2]+4, 1*hiplx[0,2]+4,0.9*hiplx[0,2]-3], [0.9*hiplx[0,2]-3, 0.9*hiplx[0,2]-3, 0.9*hiplx[0,2]-3,0.9*hiplx[0,2]-3]])
...
writergif = animation.PillowWriter(fps = 40)#10*stimfs )
