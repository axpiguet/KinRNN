...
for config in range(len(ind_configs)):
    plot_path =  f'/media/marion/PARTAGE/Documents/NeuroRestore/Results_backup/mai2021/Bars/'
    data.plot_bars(test_stim_features.iloc[np.asarray(config==reverse_configs),:], predicted_emg_array[np.asarray(config==reverse_configs),:,:], frequencies=constants.DATA_FEATURES['Frequency'], amplitudes=amplitudes_range ,norm=constants.PRE_NORMALIZATION, other_path=plot_path+'predicted/')
    data.plot_bars(test_stim_features.iloc[np.asarray(config==reverse_configs),:], test_femg_array[np.asarray(config==reverse_configs),:,:], frequencies=constants.DATA_FEATURES['Frequency'], amplitudes=amplitudes_range ,norm=constants.PRE_NORMALIZATION, other_path=plot_path+'measured/')