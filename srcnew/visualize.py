# This code is made to create .gif  of kinematics on 1 leg
...
def animate_func(num):
    ...
    for r in range(7):
        true_line = ax[1].plot(
            time[start + shift : end + shift],
            trial[start + shift : end + shift, r] - r * 2,
            color="#dbdbdd",
            linewidth=1,
        )
        predicted_line = ax[1].plot(
            time[start + shift : end + shift],
            pred_trial[start + shift : end + shift, r] - r * 2,
            "--",
            color="#dbdbdd",
            alpha=0.8,
            linewidth=1.8,
...
def plot_electrode(ax: matplotlib.axes.Axes, cathodes: int, anodes: List[int]):
    ...
    for x0, y0 in zip(x_anodes, y_anodes):
        ...
        rect3 = matplotlib.patches.Rectangle(
            (x0 - 14, y0 - 40), 36, 80, clip_box=ab, color="#8A8A8C", capstyle="round"
        )
        ax.add_patch(rect3)
    # Cathode
...
)  # add nan before and after emg_trial and time
