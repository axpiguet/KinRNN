...
si_array = np.array([controller.get_SI(predicted_emg_array, muscle=muscle, reducted_axis=1) for muscle in data.MUSCLES]).T
true_si_array = np.array([controller.get_SI(test_femg_array, muscle=muscle, reducted_axis=1) for muscle in data.MUSCLES]).T
test_femg_array/=100
...
#region do each plot 
_, ind_configs, reverse_configs = np.unique(test_stim_features.drop(['Amplitude', 'Frequency', 'Delay','Cathodes','Anodes'  ], axis=1), axis=0, return_index=True, return_inverse=True)

for config in range(len(ind_configs)):
    plot_path =  f'/media/marion/PARTAGE/Documents/NeuroRestore/Results_backup/mai2021/SI_bars/'
    data.plot_si_bars(test_stim_features.iloc[np.asarray(config==reverse_configs),:], si_array[np.asarray(config==reverse_configs),:], frequencies=constants.DATA_FEATURES['Frequency'], amplitudes=amplitudes_range ,norm=constants.PRE_NORMALIZATION, other_path=plot_path+'predicted/')
    data.plot_si_bars(test_stim_features.iloc[np.asarray(config==reverse_configs),:], true_si_array[np.asarray(config==reverse_configs),:], frequencies=constants.DATA_FEATURES['Frequency'], amplitudes=amplitudes_range ,norm=constants.PRE_NORMALIZATION, other_path=plot_path+'measured/')
