...
print(emgold.Pulses.unique())
#kinold.Amplitude = kinold.Amplitude.astype(int)

print(len(emgold['ElectrodeConftxt'].unique()))
print(emgold['Cathodes'].astype(str).unique())

fsemg = 1259.25925925926
fskin = 148.148
stimfs = 10*fskin

print(int(emgold['Cathodes'].astype(str).unique()[0].replace('[','').replace(']','')))
...
def normalize_cropped(emg_df, cols_to_cropreplace, emgfs):
    ...
    for muscle in cols_to_cropreplace :
        ...
        for row in emg_df[muscle].values:
            rows.append(100*row[nbpt_remove:])# / np.max(np.abs(row[nbpt_remove:])))
...
def draw_per_Cath(emgdf, muscle):
    ...
    fig, axes = plt.subplots(ncols=4+1, nrows=int(np.ceil(len(cathodes)/4)),figsize=(10,10),gridspec_kw={'width_ratios': [4,4,4,4,1]})
...
###
    ...
    for i,cath in enumerate(cathodes) :
        ...
        df2 = pd.crosstab(emg['Frequency'],emg['Amplitude'],values = data["".join([muscle,'max'])] , aggfunc='mean', dropna = False).fillna(0)
        sns.heatmap(df2, annot=False, cmap=cmap_, cbar_ax = axbig, ax=axes[i//4 , i % 4], vmin = 0 , vmax =100, mask = False, cbar_kws={"aspect": 100, "label": 'Max '+ muscle+' activation [%]'})
        ...
        #axes[i//4 , i % 4].tick_params(axis='x',color = 'black', pad = -1)
        if not(i%4 == 0):
    ...
    plt.tight_layout()
    #f = r"c:\Users\axell\Documents\GitHub\little_RNN\src\Datainsight\BestCathodes\\"+muscle +".png"
...
def draw_allconf(emgdf, muscle):
    ...
    fig, axes = plt.subplots(ncols=8, nrows=8,figsize=(10,10),gridspec_kw={'width_ratios': [4,4,4,4,4,4,4,1]})
...
###
    ...
    for i,conf in enumerate(confs) :
        ...
        axes[i//7 , 0].get_shared_y_axes().join(axes[i//7 , 1],axes[i//7 , 2],axes[i//7 , 3],axes[i//7 , 4],axes[i//7 , 5],axes[i//7 , 6])
        df2 = pd.crosstab(emg['Frequency'],emg['Amplitude'],values = data["".join([muscle,'max'])] , aggfunc='mean', dropna = False).fillna(0)
        sns.heatmap(df2, annot=False, cmap=cmap_, cbar_ax = axbig, ax=axes[i//7 , i % 7], vmin = 0 , vmax =100, mask = False, cbar_kws={"aspect": 100, "label": 'Max '+ muscle+' activation [%]'})
...
def draw_per_CathSPAT(emgdf, muscle):
    ...
    fig, axes = plt.subplots(ncols=4+1, nrows=9,figsize=(10,10),gridspec_kw={'width_ratios': [4,4,4,4,1]})
    gs = axes[0, 4].get_gridspec()
    for ax in axes[0:, -1]:
        ax.remove()
    axbig = fig.add_subplot(gs[0:, -1])
###
    ...
    for i,cath in enumerate(cathodes) :
        ...
        print(cathodeplot[cathode_nb])
        #axes[i//8 , 0].get_shared_y_axes().join(axes[i//4 , 1],axes[i//4 , 2],axes[i//4 , 3])
        df2 = pd.crosstab(emg['Frequency'],emg['Amplitude'],values = data["".join([muscle,'max'])] , aggfunc='mean', dropna = False).fillna(0)
        sns.heatmap(df2, annot=False, cmap=cmap_, cbar_ax = axbig, ax=axes[cathodeplot[cathode_nb]], vmin = 0 , vmax =100, mask = False, cbar_kws={"aspect": 100, "label": 'Max '+ muscle+' activation [%]'})
...
###
    ...
    for i,cath in enumerate(cathodes) :
        ...
        print(cathodeplot[cathode_nb])
        #axes[i//8 , 0].get_shared_y_axes().join(axes[i//4 , 1],axes[i//4 , 2],axes[i//4 , 3])
        df2 = pd.crosstab(emgdf['Frequency'],emgdf['Amplitude'],values = data[angle] , aggfunc='mean', dropna = False).fillna(0)
        sns.heatmap(df2, annot=False, cmap=cmap_, cbar_ax = axbig, ax=axes[cathodeplot[cathode_nb]], vmin = 0 , vmax =100, mask = False, cbar_kws={"aspect": 100, "label": 'Max '+ angle +' [degrees]'})
        axes[cathodeplot[cathode_nb]].set_title("Cathode "+cath , fontsize = 'small', y= -0.15)
        axes[cathodeplot[cathode_nb]].tick_params(axis='both', labelsize = 'x-small',colors = 'silver')
        axes[cathodeplot[cathode_nb]].tick_params(axis='y', labelrotation = 0)
        #axes[i//4 , i % 4].tick_params(axis='x',color = 'black', pad = -1)
...
def draw_per_1conf(emgdf, conf, angles , muscles):
    ...
    fig, axes = plt.subplots(ncols=8, nrows=7,figsize=(17,10),gridspec_kw={'width_ratios': [4,4,1,3,3,3,3,1]})
    gs = axes[0, 7].get_gridspec()
    for ax in axes[0:, -1]:
        ax.remove()
    axbig = fig.add_subplot(gs[0:, -1])
...
###
    ...
    for i,muscle in enumerate(muscles) :
        ...
        #axes[i//8 , 0].get_shared_y_axes().join(axes[i//4 , 1],axes[i//4 , 2],axes[i//4 , 3])
        df2 = pd.crosstab(emg['Frequency'],emg['Amplitude'],values = data["".join([muscle,'max'])] , aggfunc='mean', dropna = False).fillna(0)
        sns.heatmap(df2, annot=False, cmap=cmap_, cbar_ax = axbig1, ax=axes[i-(i // 7)*7 , i // 7], vmin = 0 , vmax =100, mask = False, cbar_kws={"aspect": 100, "label": 'Max muscle activation [%]'})
    ...
    for i,angle in enumerate(angles) :
        ...
        #axes[i//8 , 0].get_shared_y_axes().join(axes[i//4 , 1],axes[i//4 , 2],axes[i//4 , 3])
        df2 = pd.crosstab(kin['Frequency'],kin['Amplitude'],values = data[angle].abs() , aggfunc='mean', dropna = False).fillna(0)
        print(df2)
        sns.heatmap(df2, annot=False, cmap=cmap2, cbar_ax = axbig, ax=axes[row[i] , col[i]], vmin = 0 , vmax = kin[kin.columns[-6:]].max().max(), mask = False, cbar_kws={"aspect": 100, "label": 'Max angle [degree]'})
...
def draw_per_confless(emgdf, conf, angles , muscles):
    ...
    fig, axes = plt.subplots(ncols=7, nrows=7,figsize=(17,10),gridspec_kw={'width_ratios': [3,4,4,1,4,4,1]})
    ...
    for ax in axes[0:, 0]:
        ax.remove()
    #axelec = fig.add_subplot(gs[0:, 0])
    axelec = fig.add_subplot(gs[0:6, 0])
    #axelec = fig.add_subplot(gs[2:5, 0])
###
    ...
    for i,muscle in enumerate(muscles) :
        data = emg[emg['ElectrodeConftxt'] == conf]
        df2 = pd.crosstab(emg['Frequency'],emg['Amplitude'],values = data["".join([muscle,'max'])] , aggfunc='mean', dropna = False).fillna(0)
        sns.heatmap(df2, annot=False, cmap=cmap_, cbar_ax = axbig1, ax=axes[i-(i // 7)*7 , i // 7 +1], vmin = 0 , vmax =100, mask = False, cbar_kws={"aspect": 100, "label": 'Max muscle activation [%]'})
    ...
    #print(axes[0][1].get_yticks())
    row = [1,3,1,3]
    col = [3,3,4,4]

    #axes[0][3].set_xticks(axes[0][1].get_xticks() ,labels = axes[0][1].get_xticklabels())
    #axes[0][4].set_xticks(axes[0][1].get_xticks() ,labels = axes[0][1].get_xticklabels())
    for i,angle in enumerate(angles) :
        data = kin[kin['ElectrodeConftxt'] == conf]
        df2 = pd.crosstab(kin['Frequency'],kin['Amplitude'],values = data[angle].abs() , aggfunc='mean', dropna = False).fillna(0)

        sns.heatmap(df2, annot=False, cmap=cmap2, cbar_ax = axbig, ax=axes[row[i] , col[i]+1], vmin = 0 , vmax = kin[kin.columns[-6:]].max().max(), mask = False, cbar_kws={"aspect": 100, "label": 'Max angle [degree]'})
...
def draw_per_cathodeALL(emgdf, cathode, pulses, angles , muscles):
    ...
    fig, axes = plt.subplots(ncols=9, nrows=7,figsize=(17,10),gridspec_kw={'width_ratios': [5,5,5,1,3,3,3,3,1]})
...
###
    ...
    #sns.set(rc={"figure.figsize":(50, 20)})
    cmap_ = matplotlib.colors.LinearSegmentedColormap.from_list("", [background_color,'#fa525b'])
    #cmap2 = matplotlib.colors.LinearSegmentedColormap.from_list("", [background_color,'dodgerblue'])#19D3C5
    cmap2 = matplotlib.colors.LinearSegmentedColormap.from_list("", [background_color,'#19D3C5'])
    for i in range(7):
        for j in range(3):
                axes[i][j+1].set_frame_on(False)
...
def draw_per_1confelse(emgdf,kin,  conf, angles , muscles):
    ...
    fig, axes = plt.subplots(ncols=2*len(freqs)+2, nrows=len(amps),figsize=(17,10),gridspec_kw={'width_ratios': [4,4,4,4,4,4,4,4,4,4,4,4,1,1]})
    ...
    for freq in freqs :
        ...
        for amp in amps:
            i = i+1
            df2 = pd.DataFrame({'left':[0,0,0,0,0,0,0],'right':[0,0,0,0,0,0,0],'leftang':[0,0,0,0,0,0,0],'leftAdd':[0,0,0,0,0,0,0],'rightAdd':[0,0,0,0,0,0,0],'rightang':[0,0,0,0,0,0,0]})
            ...
            if len(gooddata)!=0 :
                ...
                print( gooddata[gooddata.columns[-7:]])
                for i in [0,1]:
                ...
                print(df2.info())
                print('done')
            #print(df2[['left', 'right']].values)
                sns.heatmap(df2[['left', 'right']], annot=False, cmap=cmap_, cbar_ax = axbig1, ax=axes[i ][ 2*j], vmin = 0 , vmax = 100, mask = False, cbar_kws={"aspect": 100, "label": 'Max angle [degree]'})
                print('GOTCHA')
                sns.heatmap(df2[['leftang', 'leftAdd','rightAdd', 'rightang']], annot=False, cmap=cmap2, cbar_ax = axbig, ax=axes[i ][ 2*j+1], vmin = 0 , vmax = kin[kin.columns[-6:]].max().max(), mask = False, cbar_kws={"aspect": 100, "label": 'Max angle [degree]'})
...
for side in [1]:
    for r in range(7):
        plt.plot(timeemg, 2*emg_trial[r+7*(side)]-r*230, linewidth=1, color = 'lightgrey')
